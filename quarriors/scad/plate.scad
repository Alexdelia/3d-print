...
echo(str("merged cards         ", card_block_depth(merged_card_stacks), " x ", card_block_width(),
         " x ", card_block_height()));
echo(str("base cards           ", card_block_depth(base_card_stacks), " x ", card_block_width()));
echo(str("expansion cards      ", card_block_depth(expansion_card_stacks), " x ",
         card_block_width()));
echo(str("split penalty        ",
         card_block_depth(base_card_stacks) + card_block_depth(expansion_card_stacks)
         - card_block_depth(merged_card_stacks), " mm of extra depth"));
echo(str("cards reach up to    ", card_reach()));
echo(str("plate extent         ",
         max(tray_length(6), card_block_depth(merged_card_stacks),
             card_block_depth(base_card_stacks) + plate_gap
             + card_block_depth(expansion_card_stacks)),
         " x ", split_depth + card_block_width()));
