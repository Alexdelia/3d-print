...
stack_levels = [["Coral", 5], ["MediumSeaGreen", 5], ["SteelBlue", 3]];
...
echo(str("loaded stack of ", len(stack_levels), "     rim ",
         (len(stack_levels) - 1) * tray_pitch + tray_height, ", dice top ",
         (len(stack_levels) - 1) * tray_pitch + skirt_height + floor_thickness + die_measured));
